...
def main():
    ...
    database_path = os.path.join(script_directory, "offline_tiles(gazebo).db")
    ...
    Drone_image = ImageTk.PhotoImage(Image.open(os.path.join(current_path, "images", "Drone.png")).resize((50, 50)))
    ...
    def open_file_drone():
        ...
        try:
            with open(file_name, 'r') as file:
                ...
                drone_num = int(number_string)  # Convert the string to an integer and return it
        except FileNotFoundError:
            print(f"Error: The file '{file_name}' does not exist.")
        except ValueError:
            print(f"Error: The content of the file '{file_name}' is not a valid integer.")
        ...
        for i in range(drone_num):
            try:
                with open(f'gps_data{x_coords[i]}.txt', 'r') as file:
                    ...
                    if len(parts) >= 2:
                        x, y = float(parts[0]), float(parts[1])  # Convert string to float for coordinates
                        drone += 1
                        print(f"{drone}")
                        drone_marker = map_widget.set_marker(x, y, text="Drone " + str(drone), icon=Drone_image)
    ...
    def create_rotated_images(image_path, rotations, size=(50, 50)):
        ...
        resized_image = base_image.resize(size, Image.LANCZOS)  # High-quality downsampling
        rotated_images = {}
    ...
    rotated_drone_images = create_rotated_images("map/images/Drone.png", rotations, size=(50, 50))
    ...
    def update_drone_path(drone_id):
        ...
        if len(drone_paths[drone_id]) > 1:
            ...
            yaw = calculate_bearing(latest_position[0], latest_position[1], current_position[0], current_position[1])
            ...
            latest_markers[drone_id] = map_widget.set_marker(current_position[0], current_position[1], text=f"Drone {drone_id+1}", icon=icon_image)
            if current_paths.get(drone_id):
                current_paths[drone_id].delete()
            current_paths[drone_id] = map_widget.set_path([p for p in drone_paths[drone_id]], color= colors[drone_id])
    ...
    def toggle_path_updates():
        ...
        if update_active:
            ...
            threading.Thread(target=read_points_update_paths, args=(drone_num,)).start()
